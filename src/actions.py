...
valid_anomaly_detection_comparators = [
    "LessThanLowerOrGreaterThanUpperThreshold",
    "LessThanLowerThreshold",
    "GreaterThanUpperThreshold",
]
...
def boto3_client(resource, assumed_credentials=None):
    ...
    if assumed_credentials:
        ...
        client = boto3.client(resource, config=config)
...
def check_alarm_tag(instance_id, tag_key):
    try:
        ...
        if (
            "Reservations" in instance
            and len(instance["Reservations"]) > 0
            and len(instance["Reservations"][0]["Instances"]) > 0
        ):
            ec2_client.create_tags(
                Resources=[instance_id],
                Tags=[{"Key": tag_key, "Value": str(datetime.utcnow())}],
            )
        ...
        logger.error(
            "Failure describing instance {} with tag key: {} : {}".format(
                instance_id, tag_key, e
            )
        )
        raise
...
def process_rds_alarms(
    ...
    activation_tag = [
        {"Key": activation_tag}
        for tag in tags
        if tag.get("key", None) == activation_tag
    ]
    if not len(activation_tag) > 0:
        ...
        for tag in tags:
            if tag["key"].startswith(alarm_identifier):
                logger.info(
                    "Alarm identifier found: processing db specific alarms for: {}".format(
                        db_arn
                    )
                )
                default_alarms["AWS/RDS"].append(
    ...
    if is_cluster:
        dimensions.append({"Name": "DBClusterIdentifier", "Value": db_id})
    else:
        dimensions.append({"Name": "DBInstanceIdentifier", "Value": db_id})
...
def process_lambda_alarms(
    ...
    if activation_tag == "not_found":
        logger.debug(
            "Activation tag not found for {}, nothing to do".format(function_name)
        )
        return True
    else:
        logger.debug(
            "Processing function specific alarms for: {}".format(function_name)
        )
        for tag_key in tags:
            if tag_key.startswith(alarm_identifier):
                default_alarms["AWS/Lambda"].append(
...
def create_alarm_from_tag(
    ...
    logger.info(
        "dimensions: {}, properties_offset: {}, AlarmName: {}".format(
            dimensions, properties_offset, AlarmName
        )
    )
    ...
    try:
        if additional_dimensions:
            for num, dim in enumerate(additional_dimensions[::2]):
                ...
                dimensions.append({"Name": dim, "Value": val})
...
def determine_additional_dimensions(alarm_tag, alarm_separator):
    ...
    if prop_end_index:
        ...
        logger.error(
            "Unable to determine the dimensions for alarm tag: {}".format(alarm_tag)
        )
        raise Exception
...
def process_alarm_tags(
    ...
    if create_default_alarms_flag == "true":
        ...
        if platform:
            ...
            if (
                wildcard_alarms
                and cw_namespace in wildcard_alarms
                and platform in wildcard_alarms[cw_namespace]
            ):
                for wildcard_alarm_tag in wildcard_alarms[cw_namespace][platform]:
                    ...
                    resolved_alarm_tags = determine_wildcard_alarms(
                        wildcard_alarm_tag,
                        alarm_separator,
                        instance_info,
                        metric_dimensions_map,
                    ...
                    if resolved_alarm_tags:
                        ...
                        logger.info(
                            "No wildcard alarms found for platform: {}".format(platform)
                        )
        else:
            logger.warning(
                "Skipping platform specific alarm creation for {}, unknown platform.".format(
                    instance_id
                )
            )
    else:
...
def determine_wildcard_alarms(
    wildcard_alarm_tag, alarm_separator, instance_info, metric_dimensions_map
):
    fixed_alarm_tags = []
    ...
    logger.info(
        "wildcard alarm: {}, dimensions: {}, properties offset: {}".format(
            wildcard_alarm_tag, dimensions, properties_offset
        )
    )

    wildcard_dimensions = [
        dimension for dimension in dimensions if dimension["Value"] == "*"
    ]
    fixed_dimensions = [
        dimension for dimension in dimensions if dimension["Value"] != "*"
    ]
    ...
    logger.info(
        "original dimensions: ${}, wildcard_dimensions: {},\nfixed_dimensions: {}".format(
            ...
            for wildcard_dim in wildcard_dimensions:
                ...
                for metric_dim in metric_dimensions:
                    if metric_dim["Name"] == wildcard_dim["Name"]:
                        original_dim_value_index = (
                            original_alarm_key.index(metric_dim["Name"]) + 1
                        )
                        original_alarm_key[original_dim_value_index] = metric_dim[
                            "Value"
                        ]
...
def determine_platform(imageid):
    try:
        ec2_client = boto3_client("ec2")
        # does instance have appropriate alarm tag?
        image_info = ec2_client.describe_images(ImageIds=[imageid])
        ...
        if "Images" in image_info and len(image_info["Images"]) > 0:
            ...
            if not platform and "Linux/UNIX" in platform_details:
                if "ubuntu" in image_info["Images"][0]["Name"].lower():
                    ...
                    "Description" in image_info["Images"][0]
                    and "ubuntu" in image_info["Images"][0]["Description"].lower()
                ):
                    platform = "Ubuntu"
                else:
...
def create_alarm(
    ...
    try:
        ...
        # then fail and log the exception message.
        logger.error(
            "Error converting Period specified {} to seconds for Alarm {}!: {}".format(
                Period, AlarmName, e
            )
        )
    ...
    try:
        ...
        # If any other exceptions which we didn't expect are raised
        # then fail and log the exception message.
        logger.error("Error creating alarm {}!: {}".format(AlarmName, e))
...
def delete_alarms(name, alarm_identifier, alarm_separator):
    try:
        ...
        # If any other exceptions which we didn't expect are raised
        # then fail and log the exception message.
        logger.error("Error deleting alarms for {}!: {}".format(name, e))
...
def scan_and_process_alarm_tags(
    ...
    try:
        ec2_client = boto3_client("ec2")
        default_filtered_alarms, wildcard_alarms = separate_wildcard_alarms(
            alarm_separator, cw_namespace, default_alarms
        )
...
def separate_wildcard_alarms(alarm_separator, cw_namespace, default_alarms):
    ...
    for platform in default_alarms[cw_namespace]:
        ...
        logger.info(
            "default alarms for {} are {}".format(
                platform, default_alarms[cw_namespace][platform]
            )
        )
        wildcard_alarms[cw_namespace][platform] = [
            alarm
            for alarm in default_alarms[cw_namespace][platform]
            if "*" in alarm["Key"].split(alarm_separator)
        ]
        default_alarms[cw_namespace][platform] = [
            alarm
            for alarm in default_alarms[cw_namespace][platform]
            if "*" not in alarm["Key"].split(alarm_separator)
        ]
